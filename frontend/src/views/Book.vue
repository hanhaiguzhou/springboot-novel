<template>
        ...
        <div class="book_info">
          ...
          <div class="btns" id="optBtn">
            ...
            <a
              href="javascript:void(0)"
              @click="openAiReview"
              class="btn_ora_white ai_review_btn"
              >AI评价</a
            >
            <!--
            <span id="cFavs"
                ...
                <div class="reply_btn">
                  ...
                  <span class="fr"
                    ><a
                      class="btn_ora_white ai_review_btn"
                      href="javascript:void(0);"
                      @click="aiWriteComment"
                      v-loading="aiCommentGenerating"
                      >AI帮写</a
                    ><a
                      class="btn_ora"
                      href="javascript:void(0);"
  ...
  <el-dialog
    ...
    title="AI 书评"
    width="560px"
    class="ai_review_dialog"
  >
    <div v-loading="aiReviewLoading">
      <template v-if="aiReview && aiReview.summary">
        <div class="ai_review_summary">
          <span class="ai_review_score" v-if="aiReview.score"
            >评分：{{ aiReview.score }}/10</span
          >
          <p class="ai_review_text">{{ aiReview.summary }}</p>
        </div>
        <template v-if="aiReview.pros && aiReview.pros.length">
          <h4 class="ai_review_title">亮点</h4>
          <ul class="ai_review_list">
            <li v-for="(item, index) in aiReview.pros" :key="index">
              {{ item }}
            </li>
          </ul>
        </template>
        <template v-if="aiReview.cons && aiReview.cons.length">
          <h4 class="ai_review_title">槽点</h4>
          <ul class="ai_review_list">
            <li v-for="(item, index) in aiReview.cons" :key="index">
              {{ item }}
            </li>
          </ul>
        </template>
        <p class="ai_review_audience" v-if="aiReview.audience">
          适合人群：{{ aiReview.audience }}
        </p>
        <p class="ai_review_tip">内容由 AI 生成，仅供参考</p>
      </template>
      <template v-else-if="aiReview && aiReview.rawText">
        <div class="ai_review_summary">
          <p class="ai_review_text" style="white-space: pre-wrap">{{
            aiReview.rawText
          }}</p>
        </div>
        <p class="ai_review_tip">内容由 AI 生成，仅供参考</p>
      ...
      <el-empty
        v-if="!aiReview && !aiReviewLoading"
        description="暂无数据，请稍后重试"
      />
    </div>
  </el-dialog>
  <!--AI 书评弹窗 end-->
...
import { aiBookReview, aiCommentDraft } from "@/api/ai";
...
export default {
  ...
  setup() {
    ...
    const state = reactive({
      ...
      commentId: "",
      updateComment: "",
      aiReviewDialogVisible: false,
      aiReviewLoading: false,
      aiReview: null,
      aiCommentGenerating: false,
    });
    onMounted(() => {
    ...
    const openAiReview = async () => {
      state.aiReviewDialogVisible = true;
      state.aiReviewLoading = true;
      state.aiReview = null;
      try {
        const { data } = await aiBookReview(state.book.id);
        state.aiReview = data;
      } catch (error) {
    ...
    const aiWriteComment = async () => {
      if (!state.book.id) {
        ElMessage.error("请先选择小说！");
        return;
      }
      state.aiCommentGenerating = true;
      try {
        const { data } = await aiCommentDraft(
          state.book.id,
          state.commentContent
        );
        state.commentContent = data;
        ElMessage.success("AI 草稿已生成，可修改后发表");
      } catch (error) {
        // 错误信息已由 request 拦截器统一提示
      } finally {
        state.aiCommentGenerating = false;
      }
    };
