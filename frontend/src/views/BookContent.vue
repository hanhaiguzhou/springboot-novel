  ...
  <div id="showDetail">
    <div class="readBody cf">
      <div class="readMain cf">
        ...
        <!--AI 章节导读弹窗 start-->
        <el-dialog
          v-model="aiSummaryDialogVisible"
          title="AI 本章导读"
          width="560px"
          class="ai_summary_dialog"
        >
          <div v-loading="aiSummaryLoading">
            <div
              class="ai_summary_content"
...
import { aiChapterSummary } from "@/api/ai";
...
export default {
  ...
  setup() {
    ...
    const openAiSummary = async () => {
      if (!state.data.chapterInfo) {
        ElMessage.error("章节信息加载中，请稍后再试！");
        return;
      }
      state.aiSummaryDialogVisible = true;
      state.aiSummaryLoading = true;
      state.aiSummary = "";
      try {
        const { data } = await aiChapterSummary(state.data.chapterInfo.id);
